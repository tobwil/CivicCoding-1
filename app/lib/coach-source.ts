...
export type Source = { title: string; href: string; available: boolean; description: string; materials: string; steps: string[]; preparationRaw: string; preparationMinutes: number | null; note: string; sourceRow: number };
export async function readSource(id: string, signal: AbortSignal): Promise<Source> {
  ...
  return {
    ...
    preparationRaw: source.preparationRaw, preparationMinutes: source.preparationMinutes,
